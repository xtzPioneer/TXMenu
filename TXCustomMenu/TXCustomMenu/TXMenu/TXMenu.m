...
- (void)setupMainViewFrame{
    ...
    self.scrollView.contentSize=CGSizeMake(mainViewW*_number, 0);
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    TXMenuNavigationCell * cell = self.navigationBar.cells[curIndex];
    CGRect frame=self.navigationBar.underLine.frame;
    frame.origin.x=cell.frame.origin.x;
    [self.navigationBar changeUnderlineFrameWithRect:frame];
...
- (void)menuNavigationBar:(TXMenuNavigationBar *)navigationBar clickEvent:(id)sender{
    ...
    CGFloat x=self.scrollView.frame.size.width*curIndex;
    self.scrollView.contentOffset = CGPointMake(x, 0);
